    ...
    <section id="contact" className="relative bg-[#050505] py-[80px] md:py-[140px] px-[20px] md:px-[40px] overflow-hidden">
      ...
      <div className="max-w-[1200px] mx-auto relative z-10" ref={ref}>
        ...
        <div className="flex flex-col items-center text-center gap-[16px] mb-[64px] md:mb-[80px]">
          <motion.div initial={{ opacity: 0, scale: 0.9 }} animate={inView ? { opacity: 1, scale: 1 } : {}} className="inline-flex items-center gap-[8px] px-[16px] py-[6px] rounded-full bg-[#1a1a1a] border border-[#262626]">
          ...
          <motion.h2 className="text-white font-['Poppins'] font-normal text-[40px] md:text-[56px] leading-[1.1] tracking-[-0.04em] max-w-[700px]">Let's build your <span className="text-[#32cd87]">growth engine</span> together.</motion.h2>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-[32px] md:gap-[48px] items-start relative">
          
          {/* Left Column cards remain the same ... */}
          <div className="flex flex-col gap-[16px]">
            {info.map(({ Icon, label, value, href }, i) => (
              <div key={label} className="group relative bg-[#0a0a0a] border border-[#1e1e1e] rounded-[20px] p-[24px]">
                 <div className="relative z-10 flex items-center gap-[20px]">
                  <div className="w-[56px] h-[56px] rounded-[16px] bg-[#141414] border border-[#262626] flex items-center justify-center shrink-0">
                    <Icon size={22} className="text-[#888]" />
                  </div>
                  <div className="flex flex-col gap-[4px]">
                    <span className="font-['Inter'] text-[#888] text-[13px] tracking-[0.5px] uppercase">{label}</span>
                    <span className="font-['Poppins'] text-white text-[16px]">{value}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <motion.div className="bg-[#0a0a0a] border border-[#1e1e1e] rounded-[24px] p-[24px] md:p-[40px] relative overflow-hidden shadow-[0_30px_60px_-20px_rgba(0,0,0,0.5)]">
              ...
              <form onSubmit={submit} className="flex flex-col gap-[24px]">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-[24px]">
                  <div className="flex flex-col gap-[8px]">
                    ...
                    <div className={`relative rounded-[14px] bg-[#141414] border transition-all duration-300 ${focused === 'name' ? 'border-[#32cd87]/60 shadow-[0_0_15px_rgba(37,211,102,0.1)]' : 'border-[#262626] hover:border-[#333]'}`}>
                      <input type="text" required placeholder="John Doe" value={name} onFocus={() => setFocused('name')} onBlur={() => setFocused(null)} onChange={(e) => setName(e.target.value)} className="w-full bg-transparent px-[16px] py-[14px] text-white font-['Poppins'] text-[15px] outline-none placeholder:text-[#555]" />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-[8px]">
                    ...
                    <div className={`relative rounded-[14px] bg-[#141414] border transition-all duration-300 ${focused === 'contact' ? 'border-[#32cd87]/60 shadow-[0_0_15px_rgba(37,211,102,0.1)]' : 'border-[#262626] hover:border-[#333]'}`}>
                      {method === 'whatsapp' ? (
                        <input type="tel" required placeholder="+234 800 000 0000" value={phone} onFocus={() => setFocused('contact')} onBlur={() => setFocused(null)} onChange={(e) => setPhone(e.target.value)} className="w-full bg-transparent px-[16px] py-[14px] text-white font-['Poppins'] text-[15px] outline-none" />
                      ) : (
                        <input type="email" required placeholder="john@example.com" value={email} onFocus={() => setFocused('contact')} onBlur={() => setFocused(null)} onChange={(e) => setEmail(e.target.value)} className="w-full bg-transparent px-[16px] py-[14px] text-white font-['Poppins'] text-[15px] outline-none" />
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-[8px]">
                  ...
                  <div className={`relative rounded-[14px] bg-[#141414] border transition-all duration-300 ${focused === 'message' ? 'border-[#32cd87]/60 shadow-[0_0_15px_rgba(37,211,102,0.1)]' : 'border-[#262626] hover:border-[#333]'}`}>
                    <textarea required placeholder="Tell us what you'd like to automate..." value={message} onFocus={() => setFocused('message')} onBlur={() => setFocused(null)} onChange={(e) => setMessage(e.target.value)} rows={4} className="w-full bg-transparent px-[16px] py-[14px] text-white font-['Poppins'] text-[15px] outline-none placeholder:text-[#555] resize-none" />
                  </div>
                </div>
            ...
            {submitted && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-12 text-center">
                 ...
                 <div className="text-white font-['Poppins']">Message received! Check your email for Zira's reply.</div>
                 <button type="button" onClick={() => setSubmitted(false)} className="mt-8 text-[#32cd87] text-sm underline cursor-pointer">Send another message</button>
              </motion.div>
            )}
          </motion.div>
        </div>
